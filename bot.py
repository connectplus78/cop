...
def haber_detayini_cek(url):
  try:
    ...
    # 3. TEMİZ METİN
    metin_html = ""
    icerik_alani = soup.find("div", class_=lambda x: x and any(c in x for c in ["news-content", "content", "detail", "article", "body"])) or soup
...
def ntvspor_haber_cek():
  ...
  for kategori_adi, url in kategoriler.items():
    ...
    print(f"BİLGİ - {kategori_adi} kategorisinde toplam {len(items)} öğe bulundu.")
    items = items[:12]
  ...
  # 'polis' klasörünün içine doğru yolla kaydediyoruz
